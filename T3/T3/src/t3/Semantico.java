...
public class Semantico {
    ...
    void Cmd(LaParser.CmdContext ctx){
        ...
        if(ctx.IDENT() != null){
            ...
            if (ctx.getStart().getText().equals("usar")){
                ...
                println("Linha " + id_line +" : Item " + id +" nao declarada");
                }
    ...
    void Declaracoes(LaParser.DeclaracoesContext ctx){
       //System.out.println(ctx.getText());
        if (ctx.getStart().getText().equals("item")){
            
            String id = ctx.declaracoes_objetos().obj_item().IDENT().getText();
            int id_line = ctx.declaracoes_objetos().obj_item().IDENT().getSymbol().getLine();
            if(!pilhaDeTabelas.existeSimbolo(id)){
                pilhaDeTabelas.topo().adicionarSimbolo(id, "item");
            }
            else{
                println("Linha " + id_line +" : Item" + id +" ja declarado");
            }
            ...
            if(!pilhaDeTabelas.existeSimbolo(id)){
                ...
                if(pilhaDeTabelas.tipo(id) == "item"){
                    if( !at.equals("agua") && !at.equals("machado") && !at.equals("espada")){
